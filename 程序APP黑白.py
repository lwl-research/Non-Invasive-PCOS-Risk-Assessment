...
textarea:focus,
input:focus {{
  border-color: #4F7A83 !important;
  box-shadow: 0 0 0 1px rgba(79, 122, 131, 0.16) !important;
...
.stButton > button {{
  ...
  border: 1.5px solid #303A40 !important;
}}
...
.stButton > button:hover {{
  ...
  color: #FFFFFF !important;
  border: 1.5px solid #222B30 !important;
}}
...
# ========================
DEFAULT_INPUTS = {
    # 示例输入：普通但并非“过度健康”的模拟个体参数
    "Age_input": 27,
    "Age at menarche_input": "11–15岁",
    "Menstrual cycle regularity_input": "月经周期不规则（周期长短不定，可＜21天或＞35天）",
    "Hair loss_input": "否",

    "height_cm_input": 160.0,
    "weight_kg_input": 60.0,

    "Neck circumference_input": 32.5,
    "Waist circumference_input": 76.0,
    "Hip circumference_input": 94.0,
    "Systolic blood pressure_input": 118,
    "Diastolic blood pressure_input": 76,
    "Skeletal muscle mass_input": 22.0,
    "Percent body fat_input": 31.5,
}
...
def classify_prediction(p: float, threshold: float):
    ...
    if p >= threshold:
        return "较高风险倾向", "#A65A49"
    return "较低风险倾向", "#4F7A83"
...
def plot_pp_bar(df_plot):
    ...
    colors = np.where(x_vals >= 0, "#A65A49", "#4F7A83")
    ...
    fig.update_layout(
        ...
        font=dict(family="Microsoft YaHei, PingFang SC, Arial", size=20, color="#505960"),
        yaxis=dict(
            title="",
            type="category",
...
if submitted:
    ...
    pred_bg = "#FAF1EE" if p1 >= FIXED_THRESHOLD else "#F0F5F5"

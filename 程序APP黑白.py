...
input:focus {{
  ...
  box-shadow: 0 0 0 1px rgba(0, 114, 178, 0.16) !important;
...
def classify_prediction(p: float, threshold: float):
    ...
    if p >= threshold:
        return "较高风险倾向", "#D55E00"
    return "较低风险倾向", "#0072B2"
...
def plot_pp_bar(df_plot):
    ...
    colors = np.where(x_vals >= 0, "#D55E00", "#0072B2")
...
if submitted:
    ...
    pred_bg = "#FFF8F2" if p1 >= FIXED_THRESHOLD else "#F4F9FC"

    st.markdown(
        f"""
        <div style="
            border:1.5px solid {pred_color};
            border-radius:8px;
            padding:14px 18px;
            margin-top:16px;
            margin-bottom:10px;

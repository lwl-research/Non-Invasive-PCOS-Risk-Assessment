...
# SHAP explainer
# 修复点：
# KernelExplainer 使用二维输出，避免 reshape 报错
# ========================
def predict_positive_proba_for_shap(data_like):
    p = predict_positive_proba(data_like)
...
def get_shap_explainer():
    ...
    # 用 v2 key，避免 Streamlit session_state 里复用旧 explainer
    if "kernel_shap_explainer_v2" not in st.session_state:
        st.session_state["kernel_shap_explainer_v2"] = shap.KernelExplainer(
            predict_positive_proba_for_shap,
            background_df,
            link="identity"
        )

    return st.session_state["kernel_shap_explainer_v2"]
...
def compute_kernel_shap_probability(X_one_row):
    ...
    if shap_arr.ndim == 1:
        ...
        if shap_arr.shape[0] == 1:
            local_vals = shap_arr[0]
        elif shap_arr.shape[1] == 1:
            local_vals = shap_arr[:, 0]
        else:
            local_vals = shap_arr[0]
    else:
        local_vals = shap_arr.reshape(-1)

    # 兜底：如果维度仍然和特征数不一致，返回 None，避免网页崩溃
    if len(local_vals) != len(MODEL_FEATURES):
        return None, None
...
if submitted:
    ...
    if background_df is None:
        ...
        try:
            ...
            shap_vals = None
            st.warning("预测结果已生成，但当前未能完成局部特征贡献计算。")
            st.caption(f"SHAP 计算错误：{type(e).__name__}")
